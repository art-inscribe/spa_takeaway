              ...
              <ul>
                ...
                >
                  ...
                  <div class="content">
                    ...
                    <div class="extra"><span class="count">月售 {{food.sellCount}} 份</span><span>好评率 
                      {{food.rating}}%</span></div>
...
import ShopCart from '../../../components/ShopCart/ShopCart'
 ...
 export default {
   data () {
     return {
       scrollY:0,
       tops:[],//右侧分类的li的top值得数组
       food:{}//需要显示的food
     }
   },
   mounted () {
            console.log(this.goods)
      this.$store.dispatch('getShopGoods',()=>{//数据更新之后
        this.$nextTick(()=>{
          this._initScroll()
          this._initTops()
        })
     ...
     currentIndex(){
       ...
       //根据条件产生一个结果
        const index=tops.findIndex((value,index)=>{
          ...
          // scrollY>=当前的top值&&scrollY<下一个top
   ...
   methods: {
     ...
     _initTops(){
       ...
       Array.prototype.slice.call(lis).forEach(li=>{//slice.call()将伪数组转成数组
         top+=li.clientHeight
         tops.push(top)
       })
        //3.更新数据
